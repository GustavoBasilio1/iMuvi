...
import com.example.imuvi.databinding.ActivityMapsBinding;
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        try {
            ...
            if (addresses.size() > 0) {
                ...
                LatLng holly = new LatLng(address.getLatitude(), address.getLongitude());
                MarkerOptions markerOptions = new MarkerOptions()
                        .position(holly)
                        .title(address.getLocality());
                mMap.addMarker(markerOptions);
                mMap.moveCamera(CameraUpdateFactory.newLatLngZoom(holly, 16));
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
